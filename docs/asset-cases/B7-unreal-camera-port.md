...
UE FOV 驗證 ×1、驗收對照 ×2。
相機3(6):json 定案 ×1、JSON_PATH 換行示範 ×1、UE FOV 驗證 ×1、
驗收對照 ×2、HighResShot 比例裁切實證 ×1。
...
⚠️ 灰字 Height 15.54 是 Horizontal fit 下**沒有作用的欄位殘值**,
實際有效高度由解析度比例換算:43.77 × 1080/1920 = **24.62** —
以 json 的 24.6206 為準):
...
2. UE 端出圖/截圖也用同比例:`HighResShot 5504x3072`(顯存吃緊破圖時
   ...
   - **實測現象**:對 CAM_GEN2 下 `HighResShot 1920x1080` 會得到
     **1920×1072** — 不是 bug,是 Constrain Aspect Ratio 把截圖裁到
     filmback 比例(36/20.093 = 1.7917 → 1920/1.7917 ≈ 1072)。
     1920×1072 恰為 5504×3072 的等比縮小,幾何仍正確可用;
     但**非 16:9 相機請直接用原生比例下指令**,免得尺寸出殘數。
     相機1/2 沒此現象是因為 filmback 本來就是 16:9。

     PS 讀出的實際輸出尺寸(1920×1072 = 相機3 比例的鐵證):
     ![b7 cam3 highresshot 1072](images/b7-cam3-highresshot-1072.png)
