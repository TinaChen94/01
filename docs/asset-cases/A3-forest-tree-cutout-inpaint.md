...
**思路:** **2D 保真去背 + 補圖**(不可重繪、洋紅 `#FF00FF` 底、直接當畫面素材)。
**觸發:** 去背·退底·扣圖·透明背景 / 補圖·inpaint / 被遮擋補回·補缺角 / 多物件拆件 / 緊裁換主角 / 發光裂紋·teal-cyan 保留 / 殘留手動填色。
**引用 SKILL:** `asset-cutout`(模式 1.1)。
...
- **本文 GitHub 位置(main):**
  `https://github.com/TinaChen94/01/blob/main/docs/asset-cases/A3-forest-tree-cutout-inpaint.md`
- **日期:** 2026-07-19
...
### 3b. 右場景背景小樹(同一份 prompt 跑出兩張:失敗張 = gen-04〔場景沒清乾淨〕;成功張全乾淨但尚未上傳,上傳後命名 gen-07)
