...
### 實測結果(2026-07-20,路線 A 一次過)

- ✅ 光向/氣氛全到位:頂部中央月光暈 + 樹緣冷 rim light + 地面霧 +
  深青綠低調恐怖夜;白天高飽和綠壓乾淨,暗部紋理仍可讀
- ✅ 主體樹群與中央小徑位置/輪廓目視保持;下緣雜訊區被補成延伸地面
- 📌 觀察:原灰底「空區」(路徑盡頭)被補了霧中遠景樹線與右側石塊 —
  屬「補天空/補空區」授權範圍的合理發揮;**若之後要嚴格投影對位,
  只取有資產卡的區域使用,遠景補繪當氛圍板**
- 📌 50% 疊圖對位驗收未跑(投影回 3D 前必補跑)
