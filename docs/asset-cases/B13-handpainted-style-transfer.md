...
> **1. 輸入:地板原圖(ref 1)+ 1 參考拼圖(ref 2 = v3 產出截圖裁的 4 塊)**
>
...
#### 前置:ref 2 泛用風格板製作(從遊戲截圖裁)

**ref 2 = 材質色票板** — 從目標遊戲截圖(本例《Ruined King》×3)裁「純地面特寫」
拼成一張方圖。
...
| 來源 | 裁的位置 | 避開 |
|---|---|---|
| 圖1(青綠夜競技場) | 競技場石板地面,取角色左側和下方空地 2 塊 | 角色、火焰彈道、右上 `47` 傷害數字、圓形刻線構圖 |
| 圖2(暖色叢林) | 中右側的沙土空地 2 塊 — **這張跟土路/苔地最對口,多裁** | 三個角色、藍色氣浪、綠色浮石、四周樹葉剪影 |
| 圖3(深色石板) | 左下前景的石板地 1–2 塊 | 角色、火圈、`CRITICAL 280` 字樣 |

**怎麼拼:**
- 每塊裁約 400–600px 方塊,共 4–6 塊;
- PS 拼成方形棋盤(2×2 或 2×3),塊與塊直接相接或留細灰縫皆可;
- 輸出 1024–2048 的 1:1 正方形 = ref 2。

**三條鐵律:**
1. **禁止整張截圖直接餵** — 角色/特效/UI 數字/競技場圓形構圖都會滲進貼圖
   (B2 踩坑 #1 幾何滲漏,文字鎖壓不住)。滲漏載體是「構圖」,特寫裁掉構圖就安全。
2. **每塊不能有可辨識物件或地平線** — 只要「一塊材質」,不要「一個場景的局部」;
   角色的腳、影子邊緣都不要。
3. **色票板管「畫法」,顏色由 prompt 管** — v3 的 PALETTE 句已鎖回原圖日光綠土色,
   夜色石板塊照用沒關係(模型取的是筆觸)。若出圖被拖偏青 → 圖1/圖3 塊減量、
   以圖2 暖土塊為主。

拼好後照下方 v3 跑:ref 1 = 寫實 tile、ref 2 = 這張色票板。
