...
### 2b-stab. ⚠️ 用「完整場景圖」當 IMAGE 2 氣氛來源 = 不穩定(2026-07-28 B14 實證)

**症狀:** 同配方跑 3-4 次,構圖一直往 IMAGE 2 的版面跑,結果在「構圖對/氣氛對」之間亂跳,重跑不收斂。
**根因(三因相乘):** ① NB 無 seed(底層變異)② **IMAGE 2 是完整場景圖 → 它的構圖會滲進來**
(模型分不乾淨「只取光不取構圖」)③ 構圖鎖 vs 氣氛換的 guidance 互打。
**解法 = 把「氣氛」和「幾何」拆開,別用第二張場景圖當氣氛來源:**
- **A(最穩)= 氣氛歸 PS**:構圖對的版本進 PS 調色 + 加 god rays,零變異。
- **B(留生成)= 單張 + 文字氣氛**:只餵源圖一張(唯一幾何來源 → 構圖無第二張可跑),
  B* 氣氛用**文字鎖句**寫,不掛 IMAGE 2。移除最大變異源。
- **C(硬鎖構圖)= Route B depth 控制圖**:結構焊死幾何再套氣氛。
- 📌 **兩張圖融合只適合 IMAGE 2 = 緊裁材質/色卡(無幾何)**;IMAGE 2 = 完整場景 → 幾何必污染,改單張文字或 PS。
...
**症狀:** 全局寫了「光從中上方」,但部分樹幹受光面相反(常見右側樹)。
**根因:** ① 該卡是**鏡像/複製** → 烙進去的固有光被翻面,與全局光打架;
② 上一步為保 bark 調高的 image guidance,**同時保住了錯的固有光**。
**能不能在生成階段就鎖對?能,但要對症:**
1. **治本(最便宜)= 上游修鏡像**:引擎/PS 把該樹受光面翻回來或換非鏡像卡 → relight 無對象可打架,一次對。
2. **生成階段強制光向 = 路線 B(depth 控制圖)**:控制圖用結構強制受光面,不靠 prompt 賭;
   代價 = 重打樹幹受光面 = bark 漂移(與保細節衝突)。
3. **PS dodge & burn 收尾**:純挪高光面、零形狀風險 → 對「已生成好只差光向」最穩
   (靠走道側 dodge 冷白高光、外側 burn 深藍,吸左樹高光色當標準,柔漸變不硬邊)。
