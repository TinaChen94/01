...
- **概念圖拆 3D 資產 / 拆分出資產清單 / concept→3D 高模 / 三視圖重建 / 可拼模組** → [templates/concept-to-3d-pipeline.md](templates/concept-to-3d-pipeline.md) 預設用頂部〈主模板〉兩段(單件提取 + 3視圖,複製即用、只換 [PROP]、以原圖為準);要批次自動分類才用 Master Prompt;出圖偏了才走修正層(姿勢/造型、顏色/材質、畫風/樣式))
