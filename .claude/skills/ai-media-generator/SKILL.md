...
- **概念圖拆 3D 資產 / 拆分出資產清單 / concept→3D 高模 / 三視圖重建 / 可拼模組** → [templates/concept-to-3d-pipeline.md](templates/concept-to-3d-pipeline.md) 主控生成器(放任一張參考圖→自動出清單分 3 類 + 每物件 7 軸精確描述 SPEC + 對應去背/多視圖/3D 提示詞;含顏色/材質辨識錯誤 + 畫風/樣式漂移的修正機制)
