...
- **概念圖拆 3D 資產 / 拆分出資產清單 / concept→3D 高模 / 三視圖重建 / 可拼模組** → [templates/concept-to-3d-pipeline.md](templates/concept-to-3d-pipeline.md) 主控生成器(放任一張參考圖→自動出清單分 3 類 + 兩段式:第一次極簡提取(只填道具名、以原圖為準)→ 偏離才套修正層(7 軸 SPEC、姿勢/造型、顏色/材質、畫風/樣式辨識錯誤))
