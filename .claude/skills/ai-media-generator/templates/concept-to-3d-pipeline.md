...
**觸發詞(這樣開頭就會用到這份):** 「拆這張概念圖」「concept→3D」「三視圖重建 / turnaround / model sheet」「正交立面 / front plate」「image-to-3D 參考圖」「把 [物件] 拆出來做多視圖」 ── ⚠️ **務必附上概念圖 + 講明要哪個物件([PROP])**;沒圖這份等於沒用(image-grounded)。
