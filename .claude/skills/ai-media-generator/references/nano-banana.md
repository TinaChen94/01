...
## 物件合成進場景 (game asset / 把物件放進背景圖) — 實測通用模板

**情境：** 有一張「場景/背景圖」+ 一張「物件圖（去背或純底）」，要把物件放進場景的指定位置、指定大小，並讓它**被周圍環境的光照、材質、反射影響**，自然融入。

**核心心法（2026-06 實測）：「少寫、整包交給模型對齊」勝過「逐條指定影子方向」。**
不要逐項硬寫 cast shadow 角度 / rim light 方向 / AO 強度 — 那容易跟畫面打架且很脆。改成**只下「材質與光照向周圍對齊」的整包指令**，讓 Gemini 用世界知識自己算光影反射，融入度反而更高、更通用。位置與大小用「不改變圖2原來的大小跟位置」鎖住即可。

### ✅ 勝利模板（逐字可貼，繁中原句 — 使用者實測有效）
...
| 句子 | 作用 |
|---|---|
| `參考圖1的基本構圖（只參考構圖…並重繪一張）` | 准許模型**重繪整張**（連背景一起重畫），所以光影是「一次算出來的一致場景」，不是「貼上去再補影子」→ 這是融入感的關鍵 |
| `使用圖2的圖案` | image-grounded 鎖物件造型，不靠文字描述外觀（避免漂移） |
| `不改變圖2原來的大小跟位置` | 用一句話鎖住位置/比例，免去繁複的座標描述 |
| `game art background asset` | 一個詞定調用途 → 觸發遊戲美術資產的打光/質感先驗 |
| `符合周圍的光照、反射、漫反射` | **整包**交給模型對齊，比逐條指定影子方向通用、不打架 |

### 換場景 = 換圖1 就好

同一個物件 + 同一句 prompt，只要把**圖1 換成不同氛圍的場景**（晨光森林 / 霧林沼澤…），模型會自動把物件重打光成那個場景的調性（暖陽版 vs 冷霧長苔版），物件材質、苔蘚、青苔覆蓋都會跟著環境變。**做一套資產的多場景變體超快。**

### 仍想微調時（可選，逐步加，不要一次全上）
- 要更荒廢/長在森林裡：加 `物件上長出青苔、地衣與藤蔓，像在此處數百年`
- 接地還是飄：加 `底座與地面自然交界，苔蘚與落葉爬上底座邊緣`
- 太銳利像貼紙：加 `物件吃與背景相同的大氣霧氣與景深`
